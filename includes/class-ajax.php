<?php
/**
 * AJAX Class - FIXED SCORING VERSION
 * 
 * Handles all AJAX requests for the e-learning system
 * FIXED: Proper scoring for multi-part questions (fill-in-blanks, matching, etc.)
 */
...
class ELearning_Ajax {
    ...
    /**
     * Calculate points for multiple choice questions
     */
    private function calculateMultipleChoicePoints($question, $user_answer): array {
        ...
        // Give partial credit: +1 for each correct, -0.5 for each incorrect, minimum 0
        $points = count($correct_selections) - (count($incorrect_selections) * 0.5);
        $max_points = count($correct_answers);
        
        return [
            'max_points' => $max_points,
            'earned_points' => max(0, min($points, $max_points))
        ];
    }
